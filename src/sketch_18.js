...
// はい。トーラス作ろうと思います（おおついに！！）
// 螺旋も作りたいのだけど今は難しいかな・・

// トーラス作るためには4次方程式を解かないといけないの。
...
// これでgetTorus()の返り値とする。以上～

// とりあえずここまで。テクスチャはまた今度やろうね。
// 他の4次曲面も気になる所・・てか3次曲面も行けるんだっけ・・むぅ。
...
"}" +
"void drawPyramid(out vec4 drawer, vec3 ori, vec3 dir, vec3 a0, vec3 a1, vec3 a2, vec3 a3, vec3 bodyColor){" +
"  drawTriangle(drawer, ori, dir, a0, a1, a2, bodyColor);" +
"  drawTriangle(drawer, ori, dir, a0, a2, a3, bodyColor);" +
"  drawTriangle(drawer, ori, dir, a0, a3, a1, bodyColor);" +
"  drawTriangle(drawer, ori, dir, a1, a2, a3, bodyColor);" +
"}" +
"void drawCube(out vec4 drawer, vec3 ori, vec3 dir, vec3 a0, vec3 a1, vec3 a2, vec3 a3, vec3 bodyColor){" +
"  drawRect(drawer, ori, dir, a0, a1, a2, bodyColor);" +
"  drawRect(drawer, ori, dir, a0, a2, a3, bodyColor);" +
"  drawRect(drawer, ori, dir, a0, a3, a1, bodyColor);" +
"  vec3 b0 = a0 + (a1 - a0) + (a2 - a0) + (a3 - a0);" +
"  vec3 b1 = a0 + (a2 - a0) + (a3 - a0);" +
"  vec3 b2 = a0 + (a3 - a0) + (a1 - a0);" +
"  vec3 b3 = a0 + (a1 - a0) + (a2 - a0);" +
"  drawRect(drawer, ori, dir, b0, b1, b2, bodyColor);" +
"  drawRect(drawer, ori, dir, b0, b2, b3, bodyColor);" +
"  drawRect(drawer, ori, dir, b0, b3, b1, bodyColor);" +
"}" +
// 4x^3 - 3qx = rの解を求める。
...
// しかしこれだと位相が取れないんだよ・・みゅうう。
"vec3 getNormalOfTorus(vec3 p, vec3 c, vec3 n, float a, float b){" +
"  vec3 v = p - c;" +
"  return normalize(v * (dot(v, v) - a * a - b * b * vec3(1.0, 1.0, -1.0)));" +
"}" +
